      ...
      child: Column(
        ...
        children: [
          ...
          AppPie(
            ...
            data: [
              {
                'title': 'Income: Ksh. ${formatThousands(getTotalAmount('in'))}   ${((getTotalAmount('in') / allAmounts) * 100).toStringAsFixed(2)}%',
                'color': Colors.green,
                'value': (getTotalAmount('in') / allAmounts) * 100,
              },
              {
                'title': 'Expenses: Ksh. ${formatThousands(getTotalAmount('ex'))}   ${((getTotalAmount('ex') / allAmounts) * 100).toStringAsFixed(2)}%',
                'color': Colors.red,
                'value': (getTotalAmount('ex') / allAmounts) * 100,
              },
              {
                'title': 'Savings: Ksh. ${formatThousands(getTotalAmount('sa'))}   ${((getTotalAmount('sa') / allAmounts) * 100).toStringAsFixed(2)}%',
                'color': Colors.blue,
