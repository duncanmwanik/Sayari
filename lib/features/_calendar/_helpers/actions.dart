...
void copySessionToDates({
  ...
  try {
    await showSelectDateDialog(isMultiple: true).then((newDates) async {
      ...
      state.input.setInputData(typ: feature.calendar.t, id: sessionId, dta: sessionData);
      state.input.updateSelectedDates('set', dates: newDates);
      createItem();
      if (move) {
        deleteItemForever(type: feature.calendar.t, itemId: previousDate, subId: sessionId, files: getFiles(sessionData));
      }
    });
    //
  } catch (e) {
  ...
  try {
    await showConfirmationDialog(
      ...
      onAccept: () async {
        ...
        deleteItemForever(type: feature.calendar.t, itemId: sessionDate, subId: sessionId, files: getFiles(sessionData));
      },
    );
  } catch (e) {
