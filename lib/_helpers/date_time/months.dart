...
Map<int, String> getMonthDateMap(int year, int month) {
  ...
  for (int i = firstDateIndex; i > 0; i--) {
    monthDatesMap[firstDateIndex - i] = getDatePart(firstDay.subtract(Duration(days: i)));
  }
  int remainingDays = 42 - monthDatesMap.length;
  for (int i = 0; i < remainingDays; i++) {
    monthDatesMap[monthDatesMap.length] = getDatePart(firstDay.add(Duration(days: i)));
  }
