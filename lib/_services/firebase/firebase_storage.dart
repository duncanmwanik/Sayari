...
class CloudStorage {
  final tablesRef = FirebaseStorage.instanceFor(app: Firebase.app(), bucket: 'gs://sayaritables').ref();
  ...
  Future<bool> uploadFile({required String db, required String path, required String fileId}) async {
    try {
      if (kIsWeb) {
        (db == 'tables' ? tablesRef : usersRef).child(path).putData(fileBox.get(fileId)).snapshotEvents.listen(
              (taskSnapshot) => fileProgress(taskSnapshot, db, fileId, 'uploading'),
            );
      } else {
        (db == 'tables' ? tablesRef : usersRef).child(path).putFile(File(fileBox.get(fileId))).snapshotEvents.listen(
              (taskSnapshot) => fileProgress(taskSnapshot, db, fileId, 'uploading'),
            );
      }
      return true;
  ...
  Future<bool> downloadFile({required String db, required String fileName, required String cloudFilePath, required String downloadPath}) async {
    final fileRef = (db == 'tables' ? tablesRef : usersRef).child(cloudFilePath);
  ...
  Future<void> deleteFile({required String db, required String path}) async {
    try {
      await (db == 'tables' ? tablesRef : usersRef).child(path).delete().then((value) => printThis('DEleted file $path'));
    } on FirebaseException catch (e) {
