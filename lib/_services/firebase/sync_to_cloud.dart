...
Future<bool> syncToCloud({
  ...
  if (hasInternet) {
    ...
    try {
      ...
      bool isForSession = type == feature.sessions.t;
      //
      //
      //
      // ----------------------------------------------
      ...
      if (isNew) {
        ...
        //
          ...
          await cloudService.writeData(db: db, '$parentId/${type.isNotEmpty ? '/$type' : ''}${itemId.isNotEmpty ? '/$itemId' : ''}${subId.isNotEmpty ? '/$subId' : ''}', data);
        }
        //
        //
      }
      ...
      //
        ...
        //
        await cloudService.updateData(db: db, '$parentId/${type.isNotEmpty ? '/$type' : ''}${itemId.isNotEmpty ? '/$itemId' : ''}${subId.isNotEmpty ? '/$subId' : ''}', data);
        //
      }
      //
      //
      ...
      //
        ...
        getSplitList(keys).forEach((editedKey) async {
          ...
          if (isRemoveKey) {
            ...
            await cloudService.writeData(db: db, '$parentId/$type${itemId.isNotEmpty ? '/$itemId' : ''}${subId.isNotEmpty ? '/$subId' : ''}/$editedKey', data[editedKey] ?? '');
          }
        });
      }
      //
      ...
      //
        ...
        await cloudService.deleteData(db: db, '$parentId${type.isNotEmpty ? '/$type' : ''}${itemId.isNotEmpty ? '/$itemId' : ''}${subId.isNotEmpty ? '/$subId' : ''}');
      }
      //
      //
      // --------------------------------------------------------------------------

      // if action is to delete table, we don't log anymore
      if (db == 'tables' && type.isEmpty && isDelete) {
        log = false;
      }

      // This will log the sync operation to update listening users by updating the latest table activity version
      if (log) {
      ...
      await addToPendingActions(db: db, parentId: parentId, type: type, action: action, data: data, itemId: itemId, subId: subId, keys: keys, extras: extras, log: log);
